...
class FoodKioskApp:
    ...
    def create_food_item(self, frame, item_name, price, y_position):
        tk.Label(frame, text=f"{item_name} {price}", anchor="w").place(x=60, y=y_position)
        tk.Checkbutton(frame, text="-", command=lambda: self.decrease_quantity(item_name), anchor="w").place(x=200, y=y_position)
        tk.Label(frame, textvariable=self.order_quantities[item_name]).place(x=230, y=y_position)
        tk.Checkbutton(frame, text="+", command=lambda: self.increase_quantity(item_name), anchor="w").place(x=250, y=y_position)
